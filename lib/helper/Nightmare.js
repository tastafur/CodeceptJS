...
class Nightmare extends Helper {
  ...
  async _init() {
    ...
    this.Nightmare.action('downloadManager',
      function (ns, options, parent, win, renderer, done) {
        ...
        win.webContents.session.on('will-download',
          function (event, downloadItem) {
            debug('will-download', 'trigger will-downloads');
            ...
            let handler = function () {
              let argumentss = sliced(arguments)
                .filter(function (arg) {
                  return !!arg;
                });
              let item, path;
              if (argumentss.length === 1 && argumentss[0] === Object(argumentss[0])) {
                item = argumentss[0];
              } else if (argumentss.length === 2) {
                path = argumentss[0];
                item = argumentss[1];
              }
    ...
    this.Nightmare.action('waitDownloadsComplete', function (done) {
      ...
      let waitDownloads = function waitDownloads(self, done) {
        if (dldone()) {
          return done();
    ...
    this.Nightmare.prototype.emit = function () {
